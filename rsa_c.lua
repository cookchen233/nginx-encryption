...
function _M.sign(str)
    ...
    if C.EVP_DigestInit(md_ctx, _M.md) <= 0 then
        return ssl_err()
    end
    ...
    if C.EVP_SignFinal(md_ctx, _M.buf, len, _M.pkey) <= 0 then
        return ssl_err()
    end
...
function _M.verify(str, sig)
    ...
    local siglen = #sig
    local buf = siglen <= _M.size and _M.buf
        or ffi_new("unsigned char[?]", siglen)
    ffi_copy(buf, sig, siglen)
    if C.EVP_VerifyFinal(md_ctx, buf, siglen, _M.pkey) <= 0 then
        return ssl_err()
    end
